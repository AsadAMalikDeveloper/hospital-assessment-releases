...
//
// class AssessmentScreenOffline extends StatefulWidget {
//   final HospitalAssessmentModel hospitalAssessmentModel;
//
//   AssessmentScreenOffline({super.key, required this.hospitalAssessmentModel});
//
//   @override
//   State<AssessmentScreenOffline> createState() =>
//       _AssessmentScreenOfflineState();
// }
//
// class _AssessmentScreenOfflineState extends State<AssessmentScreenOffline> {
...
//                                                   .type_id ==
//                                               " " &&
//                                           _responseTextControllers.isNotEmpty)
...
//         //           children: [
//         //             Text(
//         //               'Solved: ${(provider.sectionList![provider.selectedIndex].questions!.where((element) => element.response!.trim() != "").length)}/${(provider.sectionList![provider.selectedIndex].questions!.length)}',
...
//   }
//
//   Future<void> completeAssessment(BuildContext context,
...
//             }
//             if (sectionModel.child != null && sectionModel.child!.isNotEmpty) {
//               for (SectionChildModel sectionChildModel in sectionModel.child!) {
//                 updateLoaderMessage(
//                     context, 'Uploading staff child section...');
//                 await provider.getStaffListOffline(
//                     context,
//                     widget.hospitalAssessmentModel.assessment_id!,
//                     widget.hospitalAssessmentModel.criteria_type_id!,
//                     widget.hospitalAssessmentModel.sp_id!,
//                     sectionChildModel.id!);
//                 final responses = gatherStaffResponses(provider);
//                 final json = generateJson(responses, provider);
//                 print(jsonEncode(json));
//                 APIResponse res = await provider.submitStaffSection(
...
//                     json);
//                 if (res.status!.toLowerCase() != 'success') {
//                   toast.showErrorToast('STAFF CHILD ${res.message}' ?? '');
//                 }
//                 await Future.delayed(Duration(seconds: 1)); // Adding delay
//               }
//             }
...
//             if (sectionModel.child != null && sectionModel.child!.isNotEmpty) {
//               for (SectionChildModel sectionChildModel in sectionModel.child!) {
//                 updateLoaderMessage(context, 'Uploading bed section...');
//                 await provider.getBedCapacityListOffline(
//                     context,
//                     widget.hospitalAssessmentModel.assessment_id!,
//                     widget.hospitalAssessmentModel.criteria_type_id!,
...
//                     json);
//                 if (res.status!.toLowerCase() != 'success') {
//                   toast.showErrorToast('BED CHILD ${res.message}' ?? '');
//                 }
//                 await Future.delayed(Duration(seconds: 1)); // Adding delay
//               }
//             }
//           } else if (sectionModel.questions != [] &&
//               sectionModel.questions!.isNotEmpty) {
//             if (sectionModel.type_id == " " &&
...
//               }
//               if (sectionModel.child != null &&
//                   sectionModel.child!.isNotEmpty) {
//                 for (SectionChildModel childModel in sectionModel.child!) {
//                   updateLoaderMessage(
//                       context, 'Uploading text child section...');
//                   final responses =
//                       gatherTextResponsesOnline(childModel.questions!);
//                   final json = generateJson(responses, provider);
//                   print(jsonEncode(json));
//                   APIResponse res = await provider.submitSection(
//                       widget.hospitalAssessmentModel.criteria_type_id!,
...
//                       json);
//                   if (res.status!.toLowerCase() != 'success') {
//                     toast.showErrorToast('TEXT CHILD ${res.message}' ?? '');
//                   }
//                   await Future.delayed(Duration(seconds: 1)); // Adding delay
//                 }
//               }
...
//                     'RESPONSE ${sectionModel.id}=${res.message}' ?? '');
//               }
//               if (sectionModel.child != null &&
//                   sectionModel.child!.isNotEmpty) {
//                 for (SectionChildModel childModel in sectionModel.child!) {
//                   updateLoaderMessage(
//                       context, 'Uploading response child section...');
//                   final responses =
//                       gatherResponsesOnline(childModel.questions!);
//                   final json = generateJson(responses, provider);
//                   print('CHILD RES ${jsonEncode(json)}');
//                   APIResponse res = await provider.submitSection(
//                       widget.hospitalAssessmentModel.criteria_type_id!,
//                       widget.hospitalAssessmentModel.assessment_id!,
//                       childModel.id!,
//                       json);
//                   if (res.status!.toLowerCase() != 'success') {
//                     toast.showErrorToast('RESPONSE CHILD ${res.message}' ?? '');
//                   }
//                   await Future.delayed(Duration(seconds: 1)); // Adding delay
//                 }
//               }
...
//         }
//
//         // After all sections are processed, complete the assessment
...
//
//   //
//   // Future<void> completeAssessment(BuildContext context, AssessmentProvider provider, String assessment_id) async {
//   //   if (await _connectivityService.checkConnection()==true) {
//   //     showLoaderDialog(context, 'Starting upload...');
//   //     final APIResponse jsonResponse = await DatabaseHelper().getALLItemList();
...
//   //             for (SectionChildModel sectionChildModel in sectionModel.child!) {
//   //               updateLoaderMessage(context, 'Uploading staff child section...');
//   //               await provider.getStaffListOffline(
//   //                   context,
//   //                   widget.hospitalAssessmentModel.assessment_id!,
//   //                   widget.hospitalAssessmentModel.criteria_type_id!,
...
//   //             }
//   //           }
//   //         } else if (sectionModel.type_id == " " && sectionModel.questions![0].question_type == 'files') {
//   //           updateLoaderMessage(context, 'Uploading pictures...');
//   //           await getPictureData(provider, 0);
//   //           await uploadPicturesOnline(context,provider, sectionModel);
//   //         } else if (sectionModel.type_id == " " && sectionModel.questions![0].question_type == 'text') {
//   //           updateLoaderMessage(context, 'Uploading text responses...');
//   //           final responses = gatherTextResponsesOnline(sectionModel.questions!);
//   //           final json = generateJson(responses, provider);
//   //           print(jsonEncode(json));
//   //           APIResponse res = await provider.submitSection(
//   //               widget.hospitalAssessmentModel.criteria_type_id!,
...
//   //           if (sectionModel.child != []) {
//   //             for (SectionChildModel childModel in sectionModel.child!) {
//   //               updateLoaderMessage(context, 'Uploading text child section...');
//   //               final responses = gatherTextResponsesOnline(childModel.questions!);
//   //               final json = generateJson(responses, provider);
//   //               print(jsonEncode(json));
//   //               APIResponse res = await provider.submitSection(
//   //                   widget.hospitalAssessmentModel.criteria_type_id!,
...
//   //           if (sectionModel.child != []) {
//   //             for (SectionChildModel childModel in sectionModel.child!) {
//   //               updateLoaderMessage(context, 'Uploading response child section...');
...
//   //       }
//   //     } else {
//   //       Navigator.of(context, rootNavigator: true).pop(); // Dismiss the dialog if no data to process
//   //     }
//   //   } else {
//   //     toast.showErrorToast('You need internet connection to upload this assessment');
//   //   }
//   // }
//
...
//
//   //
//   // completeAssessment(BuildContext context, AssessmentProvider provider, String assessment_id) async {
//   //   if (await _connectivityService.checkConnection() == true) {
//   //     showLoaderDialog(context, 'Starting upload...');
//   //     final APIResponse jsonResponse = await DatabaseHelper().getALLItemList();
//   //     if (jsonResponse.data != []) {
...
//   //         else if (sectionModel.type_id == " " && sectionModel.questions![0].question_type == 'files') {
//   //           updateLoaderMessage(context, 'Uploading pictures...');
//   //           getPictureData(provider, 0);
//   //           clearTextFields();
//   //           uploadPicturesOnline(context,provider, sectionModel);
//   //         }
//   //         else if (sectionModel.type_id == " " && sectionModel.questions![0].question_type == 'text') {
//   //           updateLoaderMessage(context, 'Uploading text responses...');
//   //           final responses =
...
//   //         }
//   //         else {
//   //           updateLoaderMessage(context, 'other responses..');
//   //           print('ssa12 ${sectionModel.questions![0].question_type}');
//   //           final responses = gatherResponsesOnline(sectionModel.questions!);
//   //           final json = generateJson(responses, provider);
//   //           print(jsonEncode(json)); // You can handle the JSON as needed
//   //           resetResponses(provider); // Reset responses after gathering
...
//   }
//
//   // uploadPicturesOnline(BuildContext context, AssessmentProvider provider,
//   //     SectionModel sectionModel) async {
//   //   for (Question question in sectionModel.questions!) {
//   //     for (PicturesSectionModel picturesSectionModel in provider.picsList!) {
//   //       print('pictures ${provider.picsList!.length}');
//   //       XFile imageFile = XFile(picturesSectionModel.doc_id!);
//   //       APIResponse? res = await provider.pickImage(
//   //           context,
//   //           widget.hospitalAssessmentModel.assessment_id!,
...
//     String url =
//         "https://hmis.slichealth.com/ords/HMIS/hospital/insert_pdf?docid=$docID";
//     try {
...
//     // print('ASASAS INDEX ${index} ${(provider.sectionList![index].questions!.length)} = ${provider.sectionList![index].questions!
//     //     .where((element) => element.response!.trim() != "" || element.response_ids!.trim()!="")
//     //     .length}');
...
//             ? widget.provider.sectionList![widget.provider.selectedIndex].id!
